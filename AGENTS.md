...
| Mark Architect/QA done | MCP `architect_qa-mark_done` — after running the full landed-base gate, reviewing the integrated surface, and fixing findings; new dispatches pause until this marker matches the latest `landed_sha`. |
...
- **Architect/QA is an enforced handoff marker.** After a project lands a newer `landed_sha`, harness pauses new dispatches with `{:architect_qa_required, status}` until the orchestrator runs the full landed-base gate, reviews/fixes the integrated surface, and calls `architect_qa-mark_done` (or `Harness.ArchitectQA.mark_done/1`). This gate only compares facts; the architect seat still performs the judgment.
