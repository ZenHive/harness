...
- **Autonomous landing (shipped, v0_9).** `Harness.Lander` is the merge-train: a run that settles green on a project with `landing_policy: :auto` + `target_branch` enqueues a landing job on the project's serialized `landing_<name>` Oban queue (limit 1). The lander rebases the run's `harness/<run-id>` branch onto `origin/<target>` in a fresh worktree, **re-verifies the integrated state** with the check stack, and fast-forward-pushes (never `--force`; the operator's checkout is untouched). `Harness.Notification` fires witness events (land / blocked / red-post-merge) to configured sinks — read-only by design, never a gate. Run recovery: `hold`/`steer`/`resume` on the gen_statem (Task 150). Cron autonomy: master + per-project toggles, persisted under `~/.harness` (Tasks 109/110).
- **Agent KPIs + capability routing (shipped, v0_10).** `Harness.ResultStore` (behaviour: File default, Postgres when `repo_enabled`) persists run records best-effort at settle time; `Harness.AgentKPI` is a pure read-only rollup (success rate, first-attempt-pass, duration p90, cost-to-green); `Harness.CapabilityScore` + the benchmark corpus give per-`{agent, domain}` scores that `dispatch-recommend` routes on (explore/exploit).
...
- **v0_10 (agent KPIs & capability routing) — 14/15:** AgentKPI + dashboard (114/115), capability domains/scoring/routing (116–122), Postgres result store (137/139). Remaining: agent-eval corpus (151, handbuild).
- **v0_11 (reviewer-pair lifecycle, phase 15) — 3/4:** `:reviewing` state (161), `review_green` + empty-diff routing (162), deletion pass (163). Remaining: docs/driver-surface (164). `docs/reviewer-pair-architecture.md` is the spec.
- **Next (v0_12, phase 16):** post-merge audit agent (170). Confirm specifics via `rmap next` — this list drifts.
