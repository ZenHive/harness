...
Tasks 1–8 are done — OTP scaffold, the `AgentAdapter` behaviour, the Claude Code adapter, worktree-per-job lifecycle, rmap task ingestion, the verification runner, and the supervised run lifecycle. Task 24 — a hand-built fix that commits the agent's work to the run branch before teardown — completes the dogfooding bootstrap (see § Dogfooding below). `ROADMAP.md` (rendered from `roadmap/tasks.toml` by `rmap`) is the source of truth for what to build next: start with `rmap next`.

## Dogfooding — harness Builds harness

From the core loop onward, harness is developed *by* harness. Task 8 (the supervised single-task run lifecycle) and Task 24 (which commits the agent's work to the run branch before teardown — without it a verified run discarded its own deliverable) together are the bootstrap; they and everything before them are hand-built in a Claude Code session. **Once the bootstrap is `done`, every remaining pending task is delivered by dispatching it through harness itself** — harness ingests the rmap task, runs a headless agent in an isolated worktree, and grades the result against the verification stack. Do not hand-build a dogfoodable task in a Claude Code session; hand-build only what harness cannot yet do for itself.

- **Cutover is the bootstrap (Tasks 8 + 24), derivable from rmap.** Once both are `done`, the next `rmap next` task is a dogfood candidate — drive it via harness, don't implement it directly.
