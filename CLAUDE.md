...
- **Non-delegatable** (`Grok`, `Antigravity`, `Pi`): `rmap delegate --to` doesn't render prompts for them, so `Roadmap.ingest(agent: …)` rejects them. **Two-step pattern**: ingest with a delegatable agent (`:claude`/`:codex`/`:cursor`), then pass the `%Harness.Roadmap.Item{}` directly to the non-delegatable adapter via `Run.Supervisor.start_run/4` or `Batch`. Don't silently exclude them from batches — rotate in for adapter coverage. (The flat `dispatch__task` MCP tool hides this two-step internally.)
...
- **In progress (v0_8):** dashboard LiveViews (`Transcript.Parser` + per-agent parsers, Components/Tokens refactor, flat `dispatch__task` tool, run kill button (94), per-stack `workdir` (92), playbook prefill chips); `RoadmapLive` next. Confirm specifics via `rmap next` — this list drifts.
