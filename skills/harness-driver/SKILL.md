...
**Routing facts — `routing-brief`.** The task-writer/orchestrator resolves `assignee` + `model` from `routing-brief` over MCP/chat, not by reading `lib/harness/*.ex`. The brief joins the existing facts into one cheap read: `agents-list` roster facts, `model_availability-*` availability/blocks, and `result_store-aggregate_by_agent` KPI rollups. It returns raw per-`{agent, model}` maps only: blocked pairs are annotated with reason/source, measured KPI numbers carry `n`, and unmeasured KPI cells surface `n: 0` / `explore_candidate: true`. It does **not** compute a winner, rank, route, weighted score, or recommendation; use `dispatch-recommend` when you want the scout's advisory pick on top of these facts.
