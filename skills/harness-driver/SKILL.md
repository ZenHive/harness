...
**Routing facts — `routing-brief`.** The task-writer/orchestrator resolves `assignee` + `model` from `routing-brief` over MCP/chat, not by reading `lib/harness/*.ex`. The brief is the thin list/index: by default it returns only dispatchable pairs whose agent is installed, enabled, and available and whose `{agent, model}` is currently unblocked/available. Pass `include_all: true` only when you need the verbose full catalog, including blocked/disabled/uninstalled pairs; pass `agents: ["codex", "cursor"]` to narrow the list; pass `fields: ["agent", "model", "availability", "kpi"]` for a token-cheap pair-key projection. `domains` still scopes/annotates KPI cells only. For per-agent or per-facet depth, drill into the existing aggregate lenses (`result_store-aggregate_by_agent`, `result_store-aggregate_by_facet`, `result_store-aggregate_reviewer_reliability`, `result_store-aggregate_ceremony_cost`) using the list→get pattern. Do **not** add new KPI tools for detail those aggregate lenses already provide. The brief does **not** compute a winner, rank, route, weighted score, or recommendation; use `dispatch-recommend` when you want the scout's advisory pick on top of these facts.
