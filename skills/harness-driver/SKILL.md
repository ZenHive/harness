...
| `dispatch-coalesce` | Run an explicit list of small, related task ids in one worktree, implementer session, reviewer gate, and landing unit. Use it for tasks sharing a bundle or surface; use `dispatch-bundle` for independent work that can run in parallel. |
