...
- **Agent KPIs / routing:** `routing-brief` (THE task-writer source for assignee+model facts: roster + model availability/blocks + legacy capability cells + KPI rollups per `{agent, model}`, every metric with `n`, no ranking), `result_store-aggregate_by_agent` (per-agent KPI rollup, incl. `reviewer_flaked` — review_stuck runs excluded from the implementer's success denominator), `result_store-aggregate_reviewer_reliability` (per-reviewer-adapter rejection + no-verdict/review_stuck rate), `result_store-aggregate_ceremony_cost` (per-approved-run ceremony token facts: implementer + reviewer + audit [audit `0` until audit capture lands]; raw median/p90 distribution, no batching verdict), `result_store-get_capability_score` / `result_store-list_capability_scores` (legacy composite cells, import-only), `dispatch-recommend` (scout per-facet assessment match).
...
**Routing facts — `routing-brief`.** The task-writer/orchestrator resolves `assignee` + `model` from `routing-brief` over MCP/chat, not by reading `lib/harness/*.ex`. The brief joins the existing facts into one cheap read: `agents-list` roster facts, `model_availability-*` availability/blocks, `result_store-list_capability_scores` legacy capability cells, and `result_store-aggregate_by_agent` KPI rollups. It returns raw per-`{agent, model}` maps only: blocked pairs are annotated with reason/source, measured capability/KPI numbers carry `n`, and unmeasured domain cells surface `n: 0` / `explore_candidate: true`. It does **not** compute a winner, rank, route, weighted score, or recommendation; use `dispatch-recommend` when you want the scout's advisory pick on top of these facts.
