...
|---|---|---|
| **Operator** | Human | Starts `iex -S mix` in the harness checkout, registers projects, watches the dashboard |
...
Harness is a multi-project OTP node with Oban dispatch, per-project queues, restart resilience, Phoenix LiveView dashboard + Oban Web, a **native MCP server** (`/harness/mcp`, flat JSON tools) AND a Tidewave MCP plug (`/tidewave/mcp`, `project_eval` escape hatch) all on one Bandit endpoint (`http://localhost:4018`), and `Oban.Cron` for autonomous polling. **The native MCP tools are the primary driver surface; `project_eval` is the escape hatch** — see § "Primary Surface" below.
...
You are an AI agent in `~/_DATA/code/myapp/` (or wherever). You want harness — which is running as a long-lived `iex -S mix` BEAM in the harness checkout — to take a task from `myapp`'s roadmap, dispatch it to a headless agent in an isolated worktree of `myapp`, have a cross-family reviewer AI gate the result (it runs `myapp`'s own checks itself and fixes inline), and give you the reviewer's verdict.

Four setup steps the consuming repo needs:

**1. Operator runs harness.** A `iex -S mix` session in the harness checkout. This boots the dashboard at `http://localhost:4018`, the **native MCP server** at `http://localhost:4018/harness/mcp` (the flat driver tools), the Tidewave MCP plug at `http://localhost:4018/tidewave/mcp` (the `project_eval` escape hatch), Oban queues, the lot. Verify by opening `http://localhost:4018/harness` in a browser.
...
```
@<harness-checkout>/skills/harness-driver/SKILL.md
```
...
You are an AI agent in the harness checkout itself, building harness with harness. The skill is already imported via `@skills/harness-driver/SKILL.md` from harness's CLAUDE.md. Harness's own `.mcp.json` wires **two** entries against its single `:4018` BEAM: a `harness` entry at `/harness/mcp` (the native flat tools → `mcp__harness__dispatch-*`) and a `tidewave` entry at `/tidewave/mcp` (the escape hatch → `mcp__tidewave__project_eval`).
...
| `dispatch-await` | Dispatch + block until settled → compact verdict summary (bounded by `timeout_ms`; on timeout returns a `:timed_out` summary, run keeps going). Use it only for runs shorter than your MCP client's idle timeout (Claude Code: 300 s); for real runs watch `origin/<target>` for the lander's commit instead — see `harness-workflow.md` § Never block on `dispatch-await*`. |
...
| `dispatch-reland` | Re-enqueue landing for a settled approved run. If `landed_sha` is already persisted, resume durable roadmap writeback from that commit and reviewer provenance even when `harness/<run-id>` is gone; otherwise rebase/push. **zero agent tokens**. `Harness.Dispatch.reland/1` → `Harness.Lander.enqueue/1`. |
...
| `dispatch-pending` / `dispatch-approve` | List / drain autonomous (cron) dispatch decisions parked for operator approval when a project's cron dispatch mode is `:manual`. Only the cron poller path is gated; interactive `dispatch-task` / `dispatch-await` are never parked. |
...
**Canonical loop, zero Elixir:** `dispatch-task` (or `dispatch-bundle`) → `dispatch-status` / `dispatch-transcript` to watch → `dispatch-verdict_detail` to read the reviewer's report on a rejected run. `dispatch-await` (single) / `dispatch-await_runs` (multi) return the verdict(s) in-band, but only for runs shorter than your MCP client's idle timeout (Claude Code: 300 s); for real runs watch `origin/<target>` for the lander's commit instead — see `harness-workflow.md` § Never block on `dispatch-await*`.
...
{:ok, run_id, pid} = Harness.Run.Supervisor.start_run(
  ...
  Harness.AgentAdapter.Codex,       # or .Cursor, .Grok, .Claude, .Antigravity, .Pi
  subscriber: self(),
...
```elixir
{:ok, results} = Harness.Batch.run(items, project, Harness.AgentAdapter.Codex,
  max_concurrency: 3,
...
`rmap delegate --to` renders a native prompt for all six harness adapters — `:claude`, `:codex`, `:cursor`, `:grok`, `:antigravity`, `:pi` — so every one is a first-class `ingest(agent: …)` target dispatched directly on its own adapter. Each agent runs its own rendered prompt: `ingest(agent: :grok)` → `start_run(item, project, Harness.AgentAdapter.Grok, ...)`.
...
- `:model` — pin a grader model id from the grader agent's live catalog (`model_availability-list_available_models`).
...
- **Recover a settled run by `run_id`:** `dispatch-resume_failed` (re-dispatch a `:failed` task off its retained branch + failure report; same-agent or `escalate`), `dispatch-reland` (re-enqueue landing; resumes writeback from persisted `landed_sha` when the run branch is gone, zero tokens).
...
**Automatic task-status writeback on the Oban path.** `dispatch-task` /
...
`roadmap-ready` / `rmap next` do not return it. An **approved-but-unlanded** run (under
...
"completed re-dispatched every tick" loop. Under `landing_policy: :pr`, an approved run
also stays `in_progress` until the GitHub PR is merged: MERGE opens the PR (run record
`pr_url`, optional `rmap status <id> in_progress --landing-ref <url>`), and
`Harness.Lander.PRPoller` later writes `done --shipped-in <merge sha>` on MERGED.
A **terminal-failure** run (including a
...
| `dispatch-await` (flat MCP) | Same dispatch, verdict **in-band** — blocks until settled (bounded by `timeout_ms`), returns a compact verdict summary instead of a `run_id` to poll. Only for runs shorter than your MCP client's idle timeout (Claude Code: 300 s); for real runs watch `origin/<target>` for the lander's commit instead. |
...
| `dispatch-pending` / `dispatch-approve` (flat MCP) | **Manual-approval cron gate:** list parked autonomous decisions (`:manual` dispatch mode projects) and approve one to drain it into the normal reviewer-gated run loop. Only cron-polled work is held; interactive dispatches are not. |
...
{:ok, run_id, _pid} =
  Harness.Run.Supervisor.start_run(
    item, project, Harness.AgentAdapter.Codex,
...
{:ok, comparison} = Harness.Batch.AgentEvaluation.compare(
  item,
  project,
  [Harness.AgentAdapter.Codex, Harness.AgentAdapter.Cursor, Harness.AgentAdapter.Grok],
  max_concurrency: 3
)
...
- **Results are delivered to the subscriber** but not automatically persisted beyond Oban job rows + the `ResultStore` `LogRecord` (Postgres by default, in-memory when `repo_enabled: false`). The `LogRecord` carries the implementer transcript AND the reviewer's verdict / report / ratings / checks / concerns / proposed tasks / warning flag + fix-diff size — read it via `dispatch-verdict_detail` / `result_store-list_run_records`. The orchestrator, not harness, decides whether a proposed task belongs in the live roadmap after the delivery lands.
...
- **Secret scrubbing**: Use the `:env` map with `false` values. When you do dispatch on `Harness.AgentAdapter.Claude` from the in-process surface, pass `env: %{"ANTHROPIC_API_KEY" => false}` so the CLI uses subscription OAuth instead of an inherited metered key (`dispatch-task` does this by default via `scrub_anthropic_key`).
...
- **Context A** (consuming repo): the consuming repo's `CLAUDE.md` imports it via `@<harness-checkout>/skills/harness-driver/SKILL.md`. The skill is shared from the harness checkout; consuming repos do not vendor it.
...
When in doubt, read the current moduledocs for `Harness.AgentAdapter`, `Harness.Run`, `Harness.Batch`, `Harness.ProjectRegistry`, and `Harness.Roadmap`, then make this skill match reality. The adapter subsystem lives in the `harness_agent_adapter` package (git dependency `https://github.com/ZenHive/harness_agent_adapter.git`); the `Harness.AgentAdapter.*` namespace is unchanged, and the conformance suite is `Harness.AgentAdapter.Testing.ConformanceCase`. Tidewave `project_eval` is the fastest verifier: `function_exported?/3`, `__info__(:functions)`, `Map.keys(Struct.__struct__())`, and `get_docs` will catch most drift in seconds.
...
### Worktree reclaim coverage

`Harness.Worktree.Reclaim.run/1` defaults to a dry-run. Its report includes
`complete?`, `inspected`, `skipped`, and `errors`; an empty `items` list proves
an empty inspected backlog only when `complete?` is true. GitHub-source projects
are explicitly skipped. Missing registry, repository, target, or Git enumeration
facts make the selected inspection incomplete. Apply returns
`{:error, {:incomplete_inspection, report}}` before any mutation in that case.
`mix harness.worktree.reclaim` prints bounded coverage facts, and `--apply`
raises on incomplete inspection. Use the existing runtime for operator inspection;
do not start a second application against shared resources.
