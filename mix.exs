defmodule Harness.MixProject do
  ...
  defp deps do
    [
      # Core
      # Shared AgentAdapter subsystem (Task 397), published on Hex.
      {:harness_agent_adapter, "~> 0.1"},
      {:descripex, "~> 1.0"},
      # Compile-time option-schema validation for the Harness.Dispatch.RunTool macro DSL.
      {:nimble_options, "~> 1.0"},
      # Insights and maintenance own bounded subprocesses directly.
      {:muontrap, "~> 1.8"},
      {:ecto_sql, "~> 3.13"},
      {:postgrex, ">= 0.0.0"},
      {:oban, "~> 2.22"},
  ...
  defp aliases do
    [
      ...
      # Focused behavior and risk-relevant tests are selected by the reviewer.
      "check.dispatch": [
        "format --check-formatted",
        "compile --warnings-as-errors"
      ],
