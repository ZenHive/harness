...
<!-- FOCUS:BEGIN -->
**Focus phase:** 22 — Live-Run Observability — stop flying blind while an agent works (33 of 37 done · 0 in progress)

**Last shipped:** Task 327 — Invert the rule-content seam + make Invocation agent-agnostic (AgentAdapter no longer names Harness.AgentRules), Task 328 — Break the Driver -> Run.Reflex -> Worktree.Isolation -> AgentAdapter dependency cycle, Task 332 — Operator 'update deps' action: dispatch a dep-bump through the agent-gate, Task 333 — Scheduled full-suite (incl. integration) health check per project -> dashboard witness, Task 335 — Dispatch 'bring project to tooling baseline' through the agent-gate, Task 336 — Rust dependency-freshness provider (cargo outdated) for the freshness fact source, Task 337 — Clean up concrete Reach smell findings: narrow rescues, MapSet membership, direct map iteration, Task 338 — Triage Reach repeated-map-shape findings into structs, explicit contracts, or documented suppressions, Task 341 — JavaScript/TypeScript dependency-freshness provider for npm/pnpm/yarn projects, Task 342 — Go dependency-freshness provider for go.mod projects, Task 343 — Run gen_statem crashes with case_clause when cancel reason is {:redispatched, run_id}, Task 346 — Port the Task-163 evidence-preserving record_run merge to ResultStore.Memory, Task 347 — Worktree sweeper is depth-blind: */* glob never finds leaked landing/audit worktrees after a crash mid-land, Task 348 — Chat session lifecycle: broadcast the :busy terminal (silent message drop) + idle-reap immortal session processes, Task 349 — ProjectRegistry.list/0 refetches landing settings once per project — batch the overlay to one read, Task 350 — AgentKPI.duration_summary/1 crashes with ArithmeticError on [] — reachable from the Postgres KPI rollup, Task 352 — Bound run_records growth: blob retention that never touches the countable KPI facts, Task 353 — Decompose the 2,890-line Run gen_statem: extract per-state event handling into satellite modules, Task 356 — Chat turn-worker crash must fail fast to the caller — monitor instead of bare receive; map ensure_session already_started, Task 357 — Decompose Harness.Run.Actions (2,575 lines) by lifecycle concern on 2026-07-07

**Up next:** Task 329 — Decision spike: extract the decoupled AgentAdapter subsystem to its own hex package? [D:2/B:3/U:3 → Eff:1.5] 🚀
<!-- FOCUS:END -->
