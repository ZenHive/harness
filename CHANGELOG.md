...
- **`harness_agent_adapter` comes from Hex (`~> 0.1`).** The package is published as 0.1.0 (including `Codex.Observer`, Task 445); `mix.exs` drops the git SHA pin.
...
- **Run Insights passes no longer die at their own deadline.** One pass is a retrieval loop of up to 32 sequential observer invocations, each of which may run to the observer CLI's own 180s ceiling, and the whole loop runs inside the document store's connection checkout. All three limits sat at 180s/240s, so every pass that needed even one retrieval read was cut off: eleven hourly passes were discarded as `:observation_timeout` on 2026-09-21, each at exactly 180s, while successful passes averaged 40s and peaked at 170s. The pass budget (`Harness.Insights.pass_timeout_ms/0`, overridable with `:insights_timeout_ms`) is now 600s, the Oban job timeout derives from it, and the store checkout stays above both. A test pins that ordering.

- **The maintenance sweep no longer works a queue of leaked test fixtures.** `Harness.Maintenance.QueueTest` restored `:repo_enabled` before unregistering its fixture project, and `ProjectRegistry.Persistence.delete/1` is a no-op once the repo is disabled — so fourteen `maintenance-queue-<uuid>` projects accumulated in the live `projects` table, were swept hourly, and failed as `source_unavailable` (they carry no target branch). The callback now unregisters first. The leaked rows and their pass documents were removed from the runtime registry.
...
- **The audit agent files tasks against the node's routing facts, not its training.** The audit prompt's discovery-filing block now carries each enabled agent's standing model and catalog ids (`Harness.Audit.routing_pins/0`) with the rule that `model` must be one of those exact ids. Same incident as above — the filed pin was a remembered name that no longer exists.

- **A reviewer facing an empty implementer diff no longer implements the task itself.** The empty-diff situation in the reviewer prompt told the reviewer "you can implement it: do the work … approve"; in the run above the implementer had produced nothing and the cross-family reviewer wrote the whole delivery (168-line diff), approved its own work, and it auto-landed with no independent review. The prompt now says: verify-and-approve only when the task is already satisfied on the base, otherwise reject and quote why the implementer produced nothing. Judgment stays with the reviewer; the gate just no longer invites it to become the implementer.

- **A reviewer running a long silent check is no longer reaped mid-review.** The progress watchdog halts an agent whose transcript shows no new tool call and whose worktree fingerprint is unchanged — which is exactly what `mix ci` / `mix precommit` / `mix dialyzer` look like. Task 181 floored the *idle* window against that failure, but the driver's unfloored 300 s `progress_timeout` default reintroduced it one layer down. `Harness.Run.Actions.Timeouts` now floors the default progress window at 15 min for the implementer, reviewer and recovery phases. Higher adapter-configured defaults and explicit per-run overrides are preserved. Observed on aave_sim `run-1789369684790-eb07f7ac`: the reviewer had recorded `precommit` and `dialyzer` green in both MIX_ENVs, started `mix ci` under its own 900 s budget, and was reflex-halted before writing its verdict — a full green review settled `:review_stuck` and was thrown away.

- **`review_stuck` causes are named instead of collapsing into `:other`.** `Harness.AgentKPI.aggregate_review_stuck_causes/1` only bucketed reviewer-selection and crash causes, so every reviewer that ran and produced nothing readable landed in `:other` — 24 of 24 records at the time of writing, hiding the dominant cause. Four buckets added: `:no_verdict_artifact`, `:malformed_verdict`, `:reviewer_idle_timeout`, `:reviewer_spawn_timeout`.

- **Ready-set reads heal stale checkouts; display reads stay local (Task 420).** Cron polling, `Roadmap.ready/1`, and `Roadmap.next_bundle/2` fetch and fast-forward clean roadmap checkouts before selecting work. If sync skips, dispatch refuses when HEAD is behind origin or currency cannot be proven; a current dirty checkout remains readable. Dashboard drilldowns pass `sync_checkout: false`, avoiding fetches and checkout writes during display refreshes.

- **Roadmap reads and writes sync the checkout before rmap runs (Task 424).** `Harness.Roadmap.ingest/2` (and `next_bundle` / `ready` / the `dispatch-*` MCP tools behind them) and the durable `mark_*` writeback now fetch and fast-forward the `roadmap_path` checkout onto its roadmap branch via `Harness.Git.TargetSync.sync_checkout/2` before shelling out to `rmap`. Dispatch reads the tasks that are actually on origin instead of whatever the node's checkout last saw, and the writeback push is a fast-forward. A dirty, diverged, detached, self-host, or non-git checkout is left alone with a witnessed skip. Ingest and writeback proceed on the on-disk state; ready-set reads apply the Task 420 currency check.

- **Cron polling no longer re-dispatches tasks that already landed (Task 419).** `Harness.Cron.RoadmapPoller` consults a synced view of the roadmap branch before building the ready set, so a task the lander already marked `done --shipped-in` on origin cannot be read as still pending from a stale local checkout. Observed on bourse 2026-08-28/29: tasks 687, 688 and 690 each landed four to five times over roughly eight hourly polls, spending twenty-odd implementer and reviewer invocations redoing work already on origin. With Task 420, clean roadmap checkouts can fast-forward before polling; dirty, diverged, detached, off-target, and self-host checkouts remain protected.

- **Project cache generations survive non-build input changes (Task 423).** `Harness.ProjectCache` recipes distinguish inputs that actually affect a build from roadmap render artifacts (`ROADMAP.md`, `roadmap/data.json`, `roadmap/tasks.toml`), so an `inputs: ["."]` recipe no longer invalidates a warm generation — and block a waiting run behind a 9-to-11-minute `mix dialyzer.json --plt` — because a task status changed. Documented in `docs/project-cache.md`; the review evidence is `docs/reviews/project-cache-423.md`.

- **Delivery staging never carries harness scaffolding, injected rules, or build artifacts (Tasks 383, 385, 417 — coalesced).** `Harness.Worktree` delivery staging drops `.harness/agent-rules.md` (harness's own dispatch-time prompt, which had landed tracked on a consumer's `main`), restores the target's `AGENTS.md` so the ephemeral harness-injected header cannot ride in a commit, and refuses symlinks and dependency/build artifacts outright. The last one was the expensive case: a delivery commit whose entire diff was `deps` as a mode-120000 symlink pointing at the dispatching checkout landed on `origin/main`, and advancing the operator's checkout onto it replaced the real `deps/` with a self-referential symlink (ELOOP), breaking every `mix` command in the base checkout. Paths already committed by the agent are dropped with `git rm --cached` rather than `git reset`, which had been silently restoring HEAD's copy.
...
- **Prepared PostgreSQL test templates (Task 422).** Projects can opt into `test_db_template` to clone an explicitly prepared, frozen template into a run-specific database before agent dispatch. Harness validates the effective Ecto partition and required extensions, reports setup failures, and guards cleanup with database ownership and a run marker. The reviewer still owns migrations and project checks. See [operator setup and live tests](docs/test-db-templates.md).

- **Landed worktree reclamation (Task 377).** Post-land cleanup retries a live-run refusal with a bounded delay so terminal linger does not permanently strand the run branch. `mix harness.worktree.reclaim` plans historical branch and filesystem-orphan cleanup; `--apply` reclaims reachable leftovers or repairs stale Git back-links. Live and retained worktrees and unlanded sole-copy branches are preserved.

- **`landing_policy: :pr` — land approved runs as a GitHub pull request (Task 426).** A third landing policy alongside `:auto` and `:manual`, requiring a non-empty `target_branch` exactly as `:auto` does. Everything before MERGE is byte-identical (implementer → cross-family reviewer gate → approve); only the MERGE stage changes: the lander force-with-lease-pushes the rebased tip to `origin/harness/<run-id>` — never the target — and opens a PR with `gh pr create --base <target> --head harness/<run-id>`. `Git.TargetSync` does not run. Roadmap writeback is deferred: the task stays `in_progress` with a landing ref, the run record stores `pr_url`, and a `:pr_opened` witness fires. `Harness.Lander.PRPoller` (Oban cron, default 5 minutes) reads `gh pr view --json state,mergeCommit,mergedAt`: MERGED performs the same three effects `:auto` does at push time (rmap `done --shipped-in <merge sha>`, post-merge audit, `:landed` witness); CLOSED-unmerged marks the task `blocked` and retains the branch — never reset to pending, never re-dispatched; OPEN is a no-op. A run is written back at most once. A missing or unauthenticated `gh` fails the landing job with a witnessed reason and never falls back to a direct push.
...
- **Coalesce-dispatch: N small same-bundle tasks as one run (Task 368).** `Harness.Dispatch.coalesce/4` (MCP `dispatch-coalesce`) takes an explicit list of task ids and runs them as ONE `Harness.Run` — one worktree, one implementer invocation carrying every member prompt, one reviewer gate, one land. The reviewer verdict records per-task outcomes so the lander advances each member's roadmap status individually against the same landing SHA; a run in which any member is not marked approved fails as a unit (no partial landing in v1). The returned `write_set` is the union of every member's `touches`/`files_to_modify`, i.e. the footprint the caller serializes the next wave against, and in-flight idempotency still holds — re-dispatching any member while the coalesced run is live returns the existing `run_id`. Harness executes the coalesce but never picks what to coalesce; that selection stays orchestrator judgment. `skills/harness-driver/SKILL.md` and `priv/includes/harness-workflow.md` document when to coalesce (small tasks sharing a bundle/surface) versus parallelize.

- **Reviewer discoveries become `proposed_tasks` proposals; the orchestrator files them post-land (Task 367).** `.harness/review.json` gains an optional `proposed_tasks` array (title, body, suggested scores/markers, evidence); `Run.Result` carries it and `dispatch-verdict_detail` exposes it after worktree teardown. Reviewer prompts/rules now instruct proposing discoveries through that array and never editing `roadmap/tasks.toml`, `roadmap/data.json`, `ROADMAP.md`, or `CHANGELOG.md` in the worktree, and run staging excludes those four paths from the deliverable commit (same mechanism as the `.harness/` exclusion) so a run diff carries only code. This closes the channel where a reviewer filing a task from a stale fork assigned colliding ids, could not see the live pending set, and made roadmap files a universal write-set overlap across otherwise disjoint waves. Harness persists proposals verbatim and files nothing — filing judgment stays with the orchestrator's own task-writing gate.

- **Lander fallback: mechanically renumber additive-only `tasks.toml` conflicts (Task 369).** When a landing rebase conflicts *only* in the roadmap render set (`roadmap/tasks.toml` plus its `data.json`/`ROADMAP.md` renders) and both sides' changes are purely additive `[[task]]` blocks, `Harness.Roadmap.TaskIdRewriter` reassigns the branch-side new ids to the next free ids on the target, re-renders via `rmap`, and completes the land. Sequential integer ids plus parallel worktrees make these collisions structural — two runs filing discoveries from the same base both claim the next id. Any non-additive conflict (edits to existing task blocks, or any code file) still retains the branch and blocks as before. String references to a renumbered id anywhere in the branch diff — CHANGELOG lines, code comments, test names — are rewritten alongside the renumber, or the land is left blocked with a report naming them; a silently broken reference is never accepted.

- **Settle-persist survives schema/DB drift — no silently lost run record (Task 370).** A settle-time `run_records` insert failure (e.g. Postgrex `undefined_column` from a pending migration) no longer drops the record: the full `%Harness.Run.LogRecord{}` spills to a durable dead-letter as safe ETF under a configurable root (`Harness.ResultStore.DeadLetter`, default `~/.harness/run_record_dead_letter/`), and `Harness.ResultStore.Replayer` — a supervised GenServer on a configurable interval (default 5 s) — retries every spilled entry against the configured store until the insert path works again. Drift is named rather than guessed: `Harness.Repo.MigrationGuard` enumerates unapplied migrations at boot and at persist-failure time, and the failure surfaces loudly through the notification sink and dashboard, not only a log line. A lander `landed_sha` writeback failure on the same run no longer crashes the landing. Mechanical throughout — harness spills, names pending migrations, and replays; applying the migration stays an operator decision.

- **Operator 'update deps' action dispatches a dep-bump through the agent-gate (Task 332).** A dashboard button and an MCP tool (`dispatch-update_deps`) dispatch a dependency-bump task per project through the normal agent-gate rather than harness mechanically running `deps.update`. The prompt embeds the Task 331 dep-freshness facts as ground truth naming what is outdated and constraint-allowed, and is built from `project.languages` through one prompt provider per ecosystem (`Harness.DependencyBump.Provider.{Elixir,Rust,Go,JavaScript}`) over a shared dispatch path. Mixed-language projects can produce one language-scoped bump task per ecosystem when the freshness facts differ; minor/patch bumps batch into one task per ecosystem while each major bump gets its own. For Elixir the prompt instructs widening an over-tight pin to idiomatic `~> x.y` instead of declaring a dep blocked, and the reviewer `check_command` for bump tasks runs the project's full suite including integration tests. Harness performs no dep update or test run itself — verification is the reviewer's.

- **Operator 'tooling baseline' action dispatches through the agent-gate (Task 335).** A dashboard button and an MCP tool (`dispatch-tooling_baseline`) dispatch a per-project tooling-baseline task through the agent-gate rather than harness editing `mix.exs`. The prompt embeds Task 334 conformance-drift facts as ground truth naming the missing deps/aliases/config for every language that has a baseline provider; languages without one are visibly skipped rather than silently treated as Elixir. The Elixir prompt instructs preferring the `vibe_kit` installer, then layering `elixir-setup` extras and wiring the `ci`/`precommit` aliases and config files. The reviewer gates that the project compiles and the newly-added gate runs green; harness installs and builds nothing itself. Shares the Task 332 dispatch plumbing rather than opening a parallel dispatch path.

- **Rust dependency-freshness provider (Task 336).** `Harness.DepFreshness.Provider.Rust` produces the same fact rows (name / current / latest / constraint-allowed) as the Elixir provider for projects whose `languages` include `:rust`, extending the Task 331/340 store, dashboard panel, and provider seam — no parallel scanner, no second panel. The `cargo outdated` mechanism is confirmed to work headlessly in a cold worktree rather than assumed, with a documented fallback. Non-Rust projects are unaffected, and harness only counts and displays the raw facts — it never auto-updates or gates on them.

- **Scheduled full-suite health-check witness per project (Task 333).** A daily Oban cron (`Harness.Cron.SuiteHealthPoller`, `0 0 * * *` on the `:cron` queue) checks out a cold detached worktree at each registered project's `origin/<target>` HEAD, bootstraps it (`deps.get`, optional `ecto.create`/`migrate` on a `_h_suite_health` partition), and runs the language-derived full suite *including* normally-excluded integration tests (`mix test.json --all --include integration`, `cargo test`, `go test ./...`, `npm test`). The raw result — pass/fail, exit code, failing-test list, base SHA, timestamp — is persisted as a witness fact in `Harness.SuiteHealthStore` (Postgres `suite_health_results`, in-memory when `repo_enabled: false`) and surfaced per-project at `/harness/health`. Strictly a sakshi sink: harness counts and displays the fact and nothing in the run lifecycle reads it — no gate, block, route, down-weight, or real-vs-flaky classifier; that judgment is left to the operator or a dispatched triage agent.

- **Run-record transcript blob retention (Task 352).** Postgres `run_records` now enforce an operator-set age bound on heavyweight transcript blobs: after each successful write, rows older than `run_records.transcript_retention_ms` (default 30 days, editable on `/harness/settings`) have only `agent_output` and `reviewer_output` nulled. The run row and every countable KPI/reviewer/facet fact column remain forever, so aggregate history is unchanged while transcript storage is bounded.

- **Antigravity is model-capable as of `agy` 1.0.10 (Task 322).** `Harness.AgentAdapter.Antigravity` now declares Google/Anthropic/OpenAI model families, threads `Invocation.model` to `agy --model`, and rejects model-less dispatches before spawn like every other model-capable adapter. Because `agy --model` silently falls back on unknown ids, the adapter validates pins against a verified catalog and `ModelAvailability` can probe `agy models` to populate Antigravity's model universe. Tests cover the model-required guard, catalog parsing, unknown-model rejection, routing brief behavior, and the `agy models` probe path.

- **False-green feedback loop for reviewers (Task 321).** A red post-merge `cold_check` now also persists `approved_then_found_red` on the approved run record, carrying the reviewer adapter/model, review facets, domains, and raw cold-check fact. `AgentKPI.aggregate_reviewer_rejections/1`, `result_store-aggregate_reviewer_reliability`, `/harness/kpi`, and `routing-brief` surface false-approval count/rate facts per reviewer/model. Harness applies no score penalty, exclusion, or prose classifier; the scout/orchestrator AI judges reviewer routing from the counted facts.

- **Post-merge cold-build witness (Task 318).** Audit worktrees are now intentionally un-warmed, so the audit AI runs the project's clean-build/check in a cold tree and reports `cold_check` (`passed`, `command`, `tail`) in `.harness/audit.json`. Harness persists that agent-written fact on the landed run record; it never shells out the build or reads an exit code. A reported red cold check files a blocked follow-up rmap task and fires a loud notification, while green is silently recorded. The audit remains post-merge best-effort: no revert, unmerge, lander re-verification, or approve/reject feedback loop.

- **Reviewer verdict contract hardened (Task 317).** Reviewer prompts now require running project checks and recording any dismissed red as structured `checks`/`concerns` facts with a reproduced mechanism. `.harness/review.json` now accepts `checks` and `concerns`; `Harness.Run.Review` parses them permissively, `LogRecord`/Postgres persist them, and approved runs with non-empty concerns or reviewer-authored `passed: false` checks surface `review_warning` in records, notifications, dispatch detail, and the dashboard without auto-blocking or classifying prose.

- **Per-run test DB isolation restores the agent-gate safety net (Task 320).** Each run now injects a DB-name-safe partition suffix into the implementer and reviewer Port env, defaulting to `MIX_TEST_PARTITION` (`tapakly_test_h_<run-suffix>` under Phoenix's default `database: "..._test#{System.get_env("MIX_TEST_PARTITION")}"` convention). `%Harness.Project{}` gains `test_db_isolation_env` for custom env names, with `false` / `"none"` as explicit opt-out for projects that manage isolation themselves. On settle, harness best-effort runs `MIX_ENV=test mix ecto.drop --quiet` from worktrees whose `config/test.exs` references the isolation env; failures are logged and never gate settle or landing. This restores the per-worktree DB provisioning lost when the 2026-06-03 agent-gate rebuild deleted `Harness.Verification` / `CheckStack`, addressing the tapakly shared-`tapakly_test` contamination first observed on task 85 at 2026-06-10 13:25:42.

- **MCP blocking tool calls stay alive with SSE keepalives (follow-up to Task 310).** A blocking `tools/call` (`dispatch-await` / `dispatch-await_runs` / `dispatch-compare` can poll up to 30 min) wrote nothing to the socket while waiting, so the MCP client's idle/read timeout tore the connection down and surfaced a bare transport error — swallowing the tool's own structured `:timed_out` envelope. When the client sends `Accept: text/event-stream`, `Harness.Dashboard.MCPPlug` now runs the tool in a supervised (`async_nolink`, so a tool crash can't take down the Bandit request process) task on `Harness.Chat.TaskSupervisor`, emits `: keepalive` SSE comment frames on a clamped positive-integer interval (default 5 s; non-positive/non-integer config falls back to the default rather than spinning), and streams the real JSON-RPC result as the terminal SSE `message` event. A client disconnect mid-wait `brutal_kill`s the streaming task but leaves the run observable via `dispatch-status`; non-SSE clients keep the unchanged synchronous JSON path. Tests cover the keepalive-then-result stream, the interval clamp, and frame ordering.

- **Run-detail tracks the verdict-recovery reviewer pass (Task 315).** When the reviewer re-prompts after a missing/timed-out verdict, the run stays in `:reviewing` but is now a *distinct* live stage: `Harness.Run` stamps a `:recovery_review` entered-at timestamp on the re-prompt and `Run.Status` surfaces the second reviewer's live `agent_os_pid` (via `active_agent_os_pid/2`) and a `:recovery_review` `agent_kind` tag (via `status_agent_kind/2`) instead of leaking the prior, timed-out pass's fields. The run-detail header relays this: the Reviewer row pulses and reads `recovery reviewer: <agent>`, and the current-stage label shows `Recovery reviewer · <duration>` clocked from the new entered-at stamp. `state_entered_at` / the timing helpers widen to `Status.state() | :recovery_review`. Counted facts only — pid, tag, and elapsed of the active pass; no verdict judged in harness code. Tests cover the stamping on re-prompt, the status field selection, and the live header render.
- **Live transcript chrome — current-activity line, last-event heartbeat, file +/- chips, turn/tool summary (Task 314).** Four count/relay additions around the run-detail transcript, all derived from the existing `@transcript_events` socket data (no backend change). `Components.transcript_chrome/1` renders a `N turns · M tool calls · K files` summary header, a pinned current-activity line that relays the newest `:assistant_tool_use` in plain language (`editing <path>` / `writing <path>` / `running <cmd>` / `calling <tool>`), and a `last output Ns ago` heartbeat. The heartbeat's arrival time is stamped in the LiveView (`last_transcript_at`) only when a `{:harness_transcript, …}` / `{:harness_transcript_events, …}` message is received, and ticks via the existing 5s `:meta_tick` — a mechanical liveness fact only, never a stuck verdict. `edited_files_live/1` chips now carry per-file `+`/`−` line deltas (or an edit count when line deltas are unavailable), accumulated by the pure `edited_file_stats/1` reducer that also backs `live_edited_files/1`. Every element counts a fact or relays the latest raw tool call — no health/stuck/progress judgment computed. Component tests cover the summary/activity/heartbeat/stat helpers; a LiveViewTest exercises the activity line, summary counts, heartbeat tick, and chip deltas on a live run.
- **Transcript legibility — fallback `:system` events coalesce into one counted, content-bearing row (Task 311).** The dashboard transcript no longer renders a content-less wall of empty `OTHER`/`unknown` eyebrow rows: consecutive fallback `:system` events (`kind: :other` / `:unknown`) and `:unknown` raw lines now collapse into a single `:system_group` block that surfaces an event count, the distinct raw `_type` values, and short truncated payload snippets (`@system_snippet_chars` cap) via `eyebrow_summary/1`, so unknown wire edges stay visible without becoming noise. A lone grouped `:unknown` with raw text degrades back to the muted `<details class="transcript-unknown">` form. The Cursor parser is also hardened: tool-call IDs now resolve through `tool_call_id/2` (`call_id` → event `toolCallId` → tool-call `toolCallId`), and `tool_call_entry/1` selects the real `*ToolCall` payload by suffix so metadata siblings (`hookAdditionalContexts`, `toolCallId`) can't shadow the actual tool name/args. Component and parser tests cover the coalesced row, the cursor round-trip with tool details intact, metadata-sibling resilience, and malformed-payload safety.
- **Run timing facts — live elapsed + per-stage durations (Task 313).** `Harness.Run.Status` and `Harness.Run.LogRecord` gain `started_at` plus a `state_entered_at` map (state → wall-clock `DateTime`); the run `gen_statem` stamps `started_at` at run start and the entered-at timestamp on each state entry (including crash-settle). The run-detail header renders a live `Elapsed` clock (driven by the existing 5s `:meta_tick`, no new timer infra), the time-in-current-stage, and per-stage duration pills from the entered-at map; settled runs show their final elapsed. `Status.from_log_record/1` tolerates the fields being absent on legacy/replayed records (nil / `%{}`, never crashes). Timestamps and durations are counted facts only — no stuck/slow/health verdict is derived in harness code. Tests cover the `gen_statem` stamping across transitions (no `Process.sleep`), `from_log_record` round-trips with and without the fields, and a live non-empty elapsed render. (Follow-up `aa47fcc` dropped the now-unreachable `maybe_capture_structured_failure` catch-all flagged by Dialyzer's `pattern_match_cov` once `data()` carried the new fields.)
- **Live run header — stage stepper + active-agent indicator + live token counter (Task 312).** The run-detail `<dl>` replaces the bare `<dd>{state}</dd>` with a horizontal `stage_stepper` rendering `Harness.Run.Status.state` as a pipeline (dispatched → running → committing → reviewing → done/failed; `:recovering`/`:held` appear only while the run is in them; landing/audit are post-settle workers, not stages). The agent active in the current stage (implementer while `:running`, reviewer while `:reviewing`, recovery while `:recovering`) is distinguished with the existing `cf-live-dot` pulse, and the live token total now follows the active stage's agent so a reviewing run parses the reviewer's stream. Pure render of state + counted token facts; nothing judged. LiveViewTest covers each state's stepper position, the active-agent marker tracking state, and a non-`—` token total.
- **`warm_paths` on the project-authoring surfaces (Task 309).** The per-project `warm_paths` list (seed gitignored worktree inputs, Task 252) is now settable from both authoring surfaces, not just `ProjectRegistry.upsert/1`: `Harness.Dispatch.register_project/8` (`dispatch-register_project`) gains an optional trailing `warm_paths` `:value` arg (default `[]`), and the `/harness/settings` project edit/create form renders a `warm_paths` textarea (newline/comma-split, blank-trimmed via `parse_warm_paths/1`, prefilled from the existing project). `preserved_project_attrs/1` no longer carries `warm_paths` since the form now supplies it. Driver skill + `register_project/7→/8` arity references updated. Tests cover the dispatch arg, the form round-trip, and the parser.
- **CodeSearch warm DuckDB server — per-query boot/teardown removed (Task 308).** A new long-lived `Harness.CodeSearch.Server` GenServer (supervised in `Harness.Application`) owns one QuackDB server plus one dynamic Ecto repo per index key and executes query/index functions inside its mailbox, serializing access to the single DuckDB connection while skipping the ~per-query process boot the previous `with_repo`/`with_dynamic_repo` path paid. The Exograph index handle is cached on the resource and reopened only when missing; `rebuild_index` closes the cached resource before clearing DuckDB files. `definitions/3` gains an all-definitions path (empty `name`) backed by a direct SQL query, and both the source-fact and all-definition results are mtime-keyed ETS caches. Project-explorer latency drops from ~800 ms to sub-150 ms on warm queries. Fake server/repo test doubles cover resource reuse, stale-index rebuild, and cache freshness. (Dialyzer dead-`nil`-branch in `skip/2` cleaned up in follow-up review commit b400c1a.)
- **Bare `/harness/projects` aliases the explorer index.** The dashboard router now maps the bare `/harness/projects` path to `Harness.Dashboard.Live.ProjectExplorer` `:index` (alongside the existing `/harness/projects/explore`), so the path no longer 404s. ProjectExplorerTest covers the aliased landing.
- **Project explorer reachable from the navbar (Task 305).** The dashboard navbar now links to the project-structure explorer (`/harness/projects/explore`), and a new name-less index route renders the explorer for the first registered project (or the no-projects empty state when none is registered) so the link lands somewhere useful without a project in the URL. Follow-up to the explorer LiveView (Task 303). ComponentsTest covers the navbar link; ProjectExplorerTest covers the name-less landing (first-project default + empty-state degrade).
- **Dashboard project-structure explorer (Task 303).** New read-only `Harness.Dashboard.Live.ProjectExplorer` LiveView at `/harness/projects/:name/explore` lets an operator browse a registered Elixir project's structure via `Harness.CodeSearch`: a project selector lists modules and their defs/defps (capped at 500), and selecting a symbol drills into its callers/callees (capped at 100 each). Display-only, no mutation — the operator analogue of the structural search the reviewer/audit agents get via CodeSearch (Task 302). Non-Elixir projects, unknown projects, and an unavailable Exograph index each render a clear empty-state explaining why rather than erroring; the URL syncs on project select. Matches the existing dashboard tokens/components. LiveViewTest coverage for mount, project select, symbol drill-down, and the empty-state paths.
- **Structural code-search facts over registered Elixir projects (Task 302).** New `Harness.CodeSearch` descripex surface builds and caches an Exograph DuckDB index (under `~/.harness/code_search`, staleness-gated by source mtime) and returns fact-only locations for `refresh`, `definitions`, `callers`, `callees`, and `duplicates` (ExDNA fragments). All five are exposed as Chat/MCP tools via `Harness.Manifest`. Fact-only by design — agents decide what the locations mean. Gracefully `:skipped` for non-Elixir projects or when Exograph/QuackDB/ExDNA are unavailable; an `ExAST`-based AST fallback answers definition/call-edge queries when the index returns nothing. `exograph` (plus its `hammer`/`phoenix_iconify`/`volt`/`dune` companions) added as a `dev`/`test`-only, `runtime: false` dependency. Unit coverage for the query surface, fallback paths, skip behavior, and index lifecycle.
- **Run-detail page readability: newest-first transcript, jump-nav, compact header (c3ada3b).** `/harness/runs/:run_id` now renders transcript blocks newest-turn-first (grouping stays chronological so tool_use→tool_result pairing is preserved; only the resulting blocks reverse), so reading lands on the latest activity without scrolling to the bottom. A sticky `.run-detail-nav` jumps between Run info / Task / Changed files / Transcript; low-signal run metadata (OS pid, reason, worktree path) is tucked behind a "Run internals" disclosure; the transcript flows with the page (no scroll-within-scroll trap). The jump-nav and `scroll-margin` tokens are shared with the KPI page. LiveView tests cover the new markup.
- **Agent KPI ledger split to `/harness/kpi/agents` (Task 298).** The per-agent ledger (including dynamic reviewer-rating columns) now lives on its own dedicated page as a transposed matrix: metrics are row headers (scope=row, sticky), one column per agent, stable busiest-first column order by run count (no interactive sort — reordering columns reads worse than a deterministic order). `/harness/kpi` remains the overview with fleet headline stats and secondary KPI sections, links through to the full ledger, and the new ledger page links back. LiveView tests cover the new route, empty state, navigation, and transposed matrix rendering. (Transpose polish landed immediately after as ca1e0ac.)
- **Blocking multi-run await: `dispatch-await_runs` MCP tool (Task 289).** `dispatch-await_runs(run_ids, timeout_ms \\ 1_800_000)` blocks the caller until every listed run id (sourced from dispatch-task, dispatch-bundle, supervisor-list_runs, etc.) has settled, returning `{:ok, [%{run_id, state, reason, review_verdict}]}`. Terminal runs keep their `:done`/`:failed` state and verdict; unknown ids surface `:not_found`; still-running ids surface `state: :timed_out, reason: :await_timeout` (the runs are *not* cancelled and remain observable via `dispatch-status`). Complements single-run `dispatch-await`; 250 ms poll, bounded by the caller's timeout. Unit coverage for already-settled, not-found, and Oban in-flight timeout-marker paths.
- **Settled witness event + FileSink for orchestrator wakeup (Task 294).** `Harness.Run.settle/2` now emits a `:settled` `Notification.Event` (payload is the compact map from `Dispatch.summarize_result/1`, same shape `dispatch-await` returns) for every terminal run (`:done` / `:failed`, including approved, review-rejected, timed_out etc.). `Harness.Notification.FileSink` (new) appends one JSONL line per event (ts + type + ids + outcome + summary) to a configured path (default `~/.harness/settled.jsonl`); unconfigured or blank path is silent no-op. Registered via `:notification_sinks` list alongside `CommandSink`. Both the event emission and sink are failure-isolated. `Event.summary/1` handles `:settled`; driver pattern (tail the jsonl on wake) added to `skills/harness-driver/SKILL.md`; design doc status refreshed. New unit coverage: Event settled summaries, FileSink configured/unconfigured/malformed-path, Run settle firing the witness (matching summarize projection for done+failed). Complements the pull `dispatch-await_runs` (289) for fire-and-forget orchestrator use.
- **KPI dashboard polish + JS/TS capability domains.** `/harness/kpi` now has fleet headline stats, section jump-nav, proportion bars for rate columns, and horizontal scrolling for the wide dynamic-rating ledger. `Harness.CapabilityDomain` also pre-registers `:javascript` and `:typescript` so rmap domain tags survive the existing `String.to_existing_atom/1` path instead of collapsing to `:untagged`. LiveView and capability-domain tests cover the new presentation and domain round-trip.
- **Run show page: render the focused run's roadmap task details (Task 295).** `/harness/runs/:run_id` now renders a "Task" section (title, optional D score, marker pills, acceptance-criteria list, collapsible full body) sourced by a live read-only `Roadmap.ingest/2` (project resolved from the run's Status via ProjectRegistry). Graceful degradation to no section (nil `task_item`) on blank/missing task id, unregistered project, or drifted task id — a roadmap read never crashes the page. Reuses existing `.field` / `.config-pill` markup and adds minimal `.task-*` tokens; each sub-block is conditional. Backfill record + targeted `load_task_item/2` nil-path tests in live_test.exs.

- **Write-set serialization for `dispatch-bundle` and cron ready-set (Task 292).** Tasks whose `touches ∪ files_to_modify` intersect are no longer fanned out together; `Harness.Dispatch.WriteSetPlan` produces collision-free waves and a `collisions` report. `dispatch-bundle` enqueues and returns only the first wave under a new `serialized: %{waves: [[id]], collisions: [...]}` shape; the poller applies the same plan to its autonomous ready set and logs overlaps. Updated the bundle API description, `roadmap-ready` semantics, harness-workflow.md, and harness-driver/SKILL.md. New unit + bundle collision integration tests. Directly mitigates same-file/same-function rebase conflicts that the resolver cannot auto-reconcile.

- **Lander resolver same-function conflict excerpts + clarified reland contract (Task 293).** `Resolver.build_prompt` now supplies authoritative `<<<<<<<` marker excerpts (with 4 KiB truncation) from the conflicted files and explicit guidance for same-function/same-list/registration cases to "keep every added...". `Resilience` extracts `conflict_repair_reason/2` (detailed operator repair steps) used by both autonomous `conflict_retain` and `manual_reland_conflict`; the latter now consistently witnesses the full reason. Review commit (d61d25d) recorded the 2026-06-15 ccxt-distill root cause and "manual reland still invokes resolver once" contract in the Resolver moduledoc. Added resolver tests for excerpts, truncation, no-resolver, and prompt content; resilience and lander tests updated. No behavior change to happy-path land or recovery verbs.

- **Dashboard `/harness/roadmap` enriched beyond the 4-column rollup (Task 288).** Each project row is now expandable to a read-only planning drill-down: next pending task (id + title + optional eff), blocked tasks with `blocked_reason` text, and dispatch waves (ready tasks grouped by `dep_layer`). Data is pulled exclusively through the existing `Harness.Roadmap` (next_bundle/list/ready) + `RoadmapSummary` structured surfaces (with the same Application test seams as the rollup); row expansion toggles in-memory facts only (no click-time shell or PubSub). The 30 s `:roadmap_tick` and cold-path contract are unchanged; the view renders rmap facts only (no agent routing, bespoke scoring, or judgment). LiveViewTest coverage for no-projects, rollup row, empty planning states, and populated next/blocked/waves. Follow-up to the prior extraction of the 4-col table to its own page.

- **ProjectRegistry.upsert/1 with live Oban queue scaling (Task 283).** Replace-or-insert registration: updates the in-memory registry, persists the row when `:repo_enabled`, and ensures/scales the project's Oban queue (honoring `concurrency_cap`, default 1) without restart. `dispatch-register_project` remains the scalar create path; upsert is the foundational edit surface for the phase-20 Postgres-only project configuration direction (tasks 284/285 to surface flat edit + dashboard). `ProjectRegistry.lookup`/`list` remain the single landing-overlay read boundary; config seeds missing persisted rows on first boot.
- **Dashboard: concurrency_cap editor + project register/edit/unregister form (Task 284).** Closes the last per-project config surface holes. New `Components.project_settings_card/1` (quick cap-only rows + full edit/create form) rendered on `/harness/settings`; `SettingsLive` handlers `set_concurrency` / `save_project` / `unregister_project` route through `ProjectRegistry.upsert/1` (and `unregister/1`) so Postgres is always the source of truth. Edit forms preserve non-edited fields (landing_policy, reviewer, warm_paths, pollution_allowlist); name is identity (locked in edit rows). Tests cover render, create, edit (including blanking cap), unregister.
- **Retire config/dev.local.exs (Task 285).** `config/dev.exs` reduced to bare `import Config`; `config/dev.local.exs.example` deleted (replaced by tracked `priv/repo/seeds.exs.example`). `.gitignore` now ignores `/priv/repo/seeds.exs`. `mix.exs` aliases: `ecto.setup` runs the seed when present; new `harness.seed` alias. `runtime.exs` gains `HARNESS_RUN_MEM_THRESHOLD_GB` env override (replaces old local 18 GB tuning). `ProjectRegistry` adds `warn_shadowed_config_projects/0` (emits at boot when `:projects` present under repo_enabled) + `configured_project_entries/1` thin accessor, and still supports config seeds for non-persisted or first-boot import. Docs (CLAUDE, driver skill, playbook, README, config comment) and regression test updated. `mix harness.seed` (and ecto.setup) are the run-to-apply paths; dashboard/IEx are the live paths.
- **One-call routing brief for task-writer agents (Task 266).** New `Harness.Routing` descripex surface exposes `routing-brief`, a JSON-safe MCP/chat read that joins roster facts, model availability/blocks, legacy capability cells, and per-agent KPI rollups per `{agent, model}`. Every metric cell carries `n`; missing domain cells surface `n: 0` / `explore_candidate: true`; blocked model pairs are annotated with reason/source. The brief intentionally returns raw facts only — no winner, ranking, route, weighted score, or recommendation — and the driver docs now name it as the single assignee+model fact source beneath `dispatch-recommend`.
- **Operator read-state + MCP self-description tools (Task 262).** New `Harness.Agents`, `Harness.Autonomy`, and `Harness.Describe` descripex surfaces expose JSON-safe facts over MCP/chat: agent install/availability/enablement/reviewer eligibility/model pins, ordered reviewer slate, cron autonomy status, config schema/effective values with secret redaction, and live tool catalog/schema self-description. `Harness.Config` now contributes `config-list` / `config-get`; `Harness.Manifest.describe_tool/1` is the shared resolver behind both `describe-tool` and `Harness.Chat.Tools`, keeping dispatch and self-description on one catalog.
- **Discovery filing stage: implementer/reviewer/audit agents file follow-ups via `rmap new` (Task 251).** `Harness.RmapPath` ensures the owned sibling `rmap` CLI is on PATH inside isolated worktrees (even under scrubbed `env: %{"PATH" => ...}`) for implementer, reviewer, and audit runs. The reviewer prompt and audit prompt now instruct agents to file genuine follow-ups with `rmap new --from-stdin --roadmap-path <path>` (TOML `[[task]]` fragment) instead of TODO comments or prose notes; harness itself decides nothing and reads nothing back. Canonical operational rules updated for implementers; tests cover PATH injection and rmap-parseability. Concurrent discovery filings are handled by the existing lander/rebase + Resolver path. Pure mechanical substrate + framing.
- **Per-project `warm_paths` seed gitignored worktree inputs (Task 252).** `%Harness.Project{}` carries a `warm_paths` list of repo-relative directories (persisted in Postgres via migration `20260611120000`); `Harness.Worktree.warm/2` CoW-clones them from the parent checkout into fresh worktrees alongside the global defaults (`deps`, `_build`, `priv/plts`). Run worktree creation passes `project.warm_paths` so projects with load-bearing gitignored inputs (e.g. extractor `source/` corpus) don't cold-regenerate during implementation/review; audit worktrees are intentionally cold as of Task 318.
...
- **Shared operator flash + ephemeral settings notice (Task 210).** Dashboard notices refactored to a single `Harness.Dashboard.Components.operator_flash/1` (persistent runtime state like the no-op settings-store banner + transient `{kind, msg}` from LiveView events). Mounted once in `page_shell`; all LiveViews (dashboard, settings, etc.) now route feedback through it. The "Settings are ephemeral" warning is the canonical persistent notice when `SettingsStore.configured() == false`.
...
- **`Harness.Run.Actions` decomposed by lifecycle concern (Task 357).** Task 353's extraction left the shared lifecycle helpers concentrated in `Run.Actions` at 2,575 lines — the codebase's new second-largest file and the same maintainability class the prior decomposition targeted. Actions is now 197 lines, split along its internal seams into `Harness.Run.Actions.{Worktree,Reviewing,Recovery,Settlement,Control,Discernment,Transcript,Timeouts}`, which the `States.*` modules import directly. Behavior-preserving and pure relocation, as with 353: no state, timeout, or gen_statem surface change.

- **Reach smell findings triaged and cleaned up (Tasks 337, 338).** Each Reach-flagged bare rescue now names the exception modules it intentionally handles, or was refactored so no catch-all is needed — unexpected exceptions are no longer converted to success-shaped returns. The `capability_score.ex` `Map.keys/1 |> Enum.filter` pattern iterates the map directly as `{key, value}` pairs without behavior change. The `run.ex` `scrub_github_auth_env/1` `Map.put` is recorded as a Reach false positive and left as a map: it writes `false` as a semantic value in the caller-controlled Port env contract (`%{var => value | false}`, `false` = scrub, Task 25), not as a membership set. Separately, every repeated map-shape finding from the 2026-06-30 scan was classified as shared contract, local UI literal, external payload, or transient construction data; structs were introduced only where they reduce real drift risk and match existing boundaries, and shapes intentionally kept as maps carry a narrow suppression or explanatory comment at the site so future runs see them as triaged rather than rediscovering them as untriaged debt.

- **`exograph` tracks the hex release line (`~> 0.9`) instead of a pinned GitHub SHA.** The fix that forced the GitHub HEAD pin now ships in hex `0.9.x`, so `mix.exs` drops the `github:` dep along with the explicit `hammer` / `phoenix_iconify` / `dune` dev/test deps it required — those are now pulled transitively as optional exograph deps. Dev/test-only (`runtime: false`); no runtime surface change.
...
- **Worktree CoW clone picks the flag by OS, so Linux warm copies actually reflink (Task 410).** `Worktree.clone_copy/2` shelled out to `cp -cR` — Apple's clonefile flag, which GNU coreutils rejects outright, so every Linux warm copy fell through to a full `File.cp_r` byte copy no matter what the filesystem supported. The flag is now chosen from `:os.type()`: `-c` on Darwin, `--reflink=auto` on Linux (auto-degrades on a filesystem without reflink rather than failing). `File.cp_r` remains the fallback for an unknown OS, a rejected flag, or a raising `System.cmd`, so correctness never depends on the CoW path. Measured on the XFS `reflink=1` deployment volume after the fix: an 84 MB `deps/` tree clones in 0.05 s for a 3 MB `df` delta. The regression asserts the chosen flag per `:os.type()` without shelling out — the previous suite passed with the defect present because it only compared the copied tree's contents, which the fallback also satisfies.
...
- **Model-catalog overlay no longer hides new builtin ids after the first operator selection.** `catalog_universe/1` now merges the builtin seed unconditionally (not only as a no-static-catalog fallback), so a newly added id such as `claude-opus-5` still surfaces as an unselected universe entry. Antigravity's leftover literal `@known_model_ids` allowlist — which blocked live `agy --model` ids the hardcoded catalog had not enumerated — is gone; model support is gated once, by family prefix, in `AgentAdapter.validate_model/2`.

- **Postgres result store read path failed the whole query on one undecodable row (Task 365).** `ResultStore.Postgres` atomized jsonb keys on read, so a single row referencing an atom not loaded in the current BEAM made `list_run_records/0-2` — unfiltered and every filtered variant — return `{:error, %ArgumentError{message: "... not an already existing atom"}}`, taking the `result_store-list_run_records` MCP tool and the whole run ledger down with it (observed 2026-07-14 against the live dev BEAM). Row decode is now tolerant like the File store: every decodable record is returned, and skipped or degraded rows surface as one aggregated `Logger.debug` line per scan rather than a per-row warning or a whole-query error. Pinned by a real-Postgres regression test that inserts a row whose jsonb references an unknown atom-like key and asserts healthy siblings still come back.

- **Postgres result store silently ignored `:task_id` and `:landed_sha` filters.** `ResultStore.Postgres.apply_filters/2` only translated `run_id`/`batch_id`/`agent`/`adapter`/`verdict`/`project_name`, so `list_run_records` with `task_id:` (the recovery lookup `Run.Worker.recoverable_run_record/2`) or `landed_sha:` (post-merge `Audit.persist_cold_check/4`) returned every row for the project and leaned on downstream Elixir filtering — a Postgres-only divergence from the Memory backend, which matched generically all along. Filter translation is now table-driven over the string-equality columns (`run_id`, `batch_id`, `project_name`, `task_id`, `task_fingerprint`, `landed_sha`), and `Audit.persist_cold_check/4` scopes its query to the single `landed_sha` row (keeping `include_transcripts: true` so the re-write preserves blobs). Adds a `run_records` lookup-index migration (`task_id`, `landed_sha`, `verdict`, `reviewer_adapter`) and a `ResultStoreContract.assert_scoped_filters/1` regression run against both backends.

- **Two reviewer-gate trust holes from the 2026-06-20 audit (Task 319).** (1) `Harness.Lander.enqueue/1` (the `dispatch-reland` path) loaded the run record by `run_id` and ff-pushed `harness/<run-id>` to the target with no check that the run was reviewer-approved — an operator/orchestrator could reland a `reject`/`:failed` run, pushing un-approved work with zero re-verification. It now asserts the loaded record settled `:done` with `verdict: :approve` (mirroring the auto-land `reason: :approved` gate); a non-approved reland is refused with `{:error, {:not_approved, %{state, verdict}}}` and enqueues nothing. The auto-land path was already correctly gated — only the manual reland was missing the guard. (2) The fake reviewer/codex test doubles write `.harness/review.json` relative to the Port cwd; a test that misconfigured that cwd could leak an auto-`approve` verdict file into the main checkout root (one was found there on 2026-06-20). `Harness.RunCase` now guards every test with an `on_exit` that removes any repo-root `.harness/review.json` the test leaked and fails loudly. Tests cover the reland guard (refuses `:reject`/`:failed`, re-lands an approved run) and the leak guard runs clean across the RunCase suite.
- **`dispatch-bundle` honors each task's pinned `assignee` and gates `depends_on` within the wave (Task 316).** The bundle path resolved a single adapter for the whole wave and routed every task to it, ignoring per-task `assignee` pins, and `WriteSetPlan` only deduped on write-set overlap — so a task that `depends_on` another still-pending task in the same bundle could be dispatched alongside its unmet dependency. Now `Harness.Dispatch.bundle/3` filters the wave through `dependency_ready_tasks/1` (dropping any task whose `depends_on` names a non-`done` task in the set) before planning, and `ingest_bundle_task/3` resolves the adapter per task from its `assignee` (falling back to the supplied adapter for blank/`human`), pinning each item's effective model to that adapter (`effective_model_for_adapter/3` — keeps the task's `model` only when the resolved adapter supports it, else the per-agent default). The requested model is threaded through Oban via `Batch.dispatch(..., persist_requested_model: true)` → `Run.Worker` so a cross-agent bundle dispatch carries the right model. Tests cover per-task assignee routing, depends_on gating, and the model-arg threading.
- **`StatusView.describe_reason` no longer crashes the dashboard on a `{:unavailable, agent, model, opts}` settle reason.** The 4-tuple settle reason emitted by a blocked-model reviewer/dispatch rejection had no matching `describe_reason/1` clause, raising a `FunctionClauseError` that took down the run-status view. Added a clause that renders `<agent> model <model> unavailable (<available ids | none available>)` plus a defensive `inspect/1` catch-all so any future reason shape degrades to a string instead of crashing. `@spec` widened from `Result.reason()` to `term()` (the unused `Result` alias dropped). Tests cover the new clause and the fallback.

- **MCPPlug `direct_post` no longer returns an `id:null` parse error after the endpoint drains the body (follow-up to Task 310).** The dashboard endpoint runs `Plug.Parsers` (`:json`) before forwarding to `Harness.Dashboard.MCPPlug`, so the raw request body is already drained — the earlier `direct_post` re-read it, got `""`, and answered every session-bearing request (`tools/list`/`tools/call`/`ping`) with `{"error":{-32700},"id":null}`, which MCP clients reject (error `id` must be string|number, never null), breaking `/mcp`. The plug now trusts the parsed `conn.body_params` via a new `fetch_message/1`, falling back to `read_body/2` only when mounted without an upstream JSON parser. Regression test mirrors the endpoint's `Plug.Parsers` step and asserts a session-header `tools/list` returns the tool list with the correct `id`, not an `id:null` parse error.
- **`dispatch-await` attach-to-in-flight no longer wedges the MCP transport — read tools stay responsive (Task 310).** A `dispatch-await` that attached to an already-running `{project, item_id}` ran inside anubis's per-session single-flight queue, so its long poll blocked the queue: the await itself never returned the documented structured `:timed_out` summary (the MCP transport deadline fired first), and concurrent read-only tools (`dispatch-status`, `supervisor-list_runs`) starved behind it. New `Harness.Dashboard.MCPPlug` fronts the `/harness/mcp` forward: session-creation POSTs still go to the anubis StreamableHTTP plug, but session-bearing initialized tool requests (`ping`/`tools/list`/`tools/call`) dispatch directly to `Harness.Dashboard.MCPServer` in the Plug request process, so each request runs concurrently and `dispatch-await` honors `timeout_ms` by returning the structured `:timed_out` map as a normal tool result. Regression test dispatches a faked in-flight run, awaits with a short `timeout_ms`, and asserts the structured `:timed_out` summary returns while concurrent `dispatch-status`/`supervisor-list_runs` on the same session respond promptly.
- **Cron poller suppresses re-dispatch of a task whose adapter or model is unavailable (Task 304).** The autonomous ready-set path now gates each routed task through `ensure_adapter_available/2` + `ensure_model_available/4` before enqueue: a task whose resolved adapter has no available agent, or whose effective model (task `model` when it matches the task's assignee, else the per-agent default) is blocked/unavailable, is logged at info as a dispatch suppression and skipped instead of being enqueued and bounced. Closes the churn loop where the poller re-woke the same un-dispatchable task every cron tick. The ready-set projection now also carries `model` so the model gate sees the task's pin. Poller tests cover the adapter-unavailable and model-unavailable suppression paths.
- **`dispatch-await` attaches to an in-flight run instead of duplicate-dispatching.** `Dispatch.await/5` now routes through the same Oban-guarded `enqueue_start/4` path as `dispatch-task`, so an await for a `{project, item_id}` already running attaches to the existing run (the unique guard returns its run_id) rather than starting a second concurrent run, then polls `status/1` to settle and rehydrates the rich verdict summary (verdict/report/ratings + diff sizes) from the persisted record. Removes the bespoke direct-start + mailbox-receive path (`start/5`); the subscriber-push mechanism remains for other callers. Terminal-but-unreadable and never-persisted ("vanished") runs degrade to a status-snapshot summary. `dispatch_test.exs` updated for the poll/attach path.
- **Run gen_statem survives BEAM code reload / node restart during review (Task 299).** A hot code reload or BEAM restart while a run is in `:reviewing` (or `:recovering` / `:held`) can produce a classic `:undef` (or `:function_clause`) on the state callback (observed on the `transcript_chunk` :info path). `Harness.Run.terminate/3` now classifies such crashes in the recoverable states via `crash_settle` + `normalize_crash_reason` as `{:run_crashed, {:code_reload, state, detail}}`, persists a minimal failed `Result` record (so the failure is visible in ResultStore/KPIs without digging Oban), finishes the worktree, and notifies the subscriber. The `Run.Worker` guards `revert_to_pending` so a recoverable crash with a retained `harness/<run-id>` branch leaves the task `in_progress` (recover via `dispatch-rereview`, not a fresh implementer dispatch); a warning is logged. Dedicated regression test (exact `reviewing/3` + transcript_chunk undef) plus a negative test proving a live `transcript_chunk` message does not crash on a loaded module. Added operational warning + recovery contract to `docs/dogfooding-workflow.md`. Dupe list of recoverable states between Run and Worker noted as follow-up (filed Task 300).

- **Consolidate `@recoverable_code_reload_states` to single source of truth in `Harness.Run` (Task 300).** `Run` now defines the list (with updated comment) and exposes `recoverable_code_reload_states/0` (internal, `@doc false`); `Run.Worker` derives its compile-time guard constant from the function call (Task 300). Cross-references and comments updated in both modules. No behavior change.
- **list_available_models distinguishes 'no catalog' vs 'probed-but-none-selected' (Task 301).** `list_available_models/2` now returns three `catalog_status` values ("available" | "probed_none_selected" with `universe_count` | "unavailable") while keeping the legacy `catalog_unavailable` boolean for backward-compat. New private helper + three targeted unit tests (static-selected, probe-cache-but-nothing-selected, no-universe-at-all); `api/0` returns description expanded. Internal dispatch paths (`list_available/1`, `catalog/1`) and `routing` consumer unchanged.

- **MCP StreamableHTTP transport timeout now matches blocking dispatch tools (Task 296).** `Harness.Dashboard.MCPServer.request_timeout_ms/0` centralizes the 30-minute request budget and both the supervised Anubis server and `/harness/mcp` Plug forward use it. This removes the previous 30-second `GenServer.call` ceiling that killed `dispatch-await`, `dispatch-await_runs`, and `dispatch-compare` over MCP even though the tools' own `timeout_ms` defaults to 30 minutes. Regression coverage holds a real `dispatch-await_runs` request past the old 30-second ceiling.
- **Lander additive-file conflicts resolve mechanically before the agent resolver (Task 297).** Rebase conflicts on configured append-only files now run through a `git merge-file --union` path first (`CHANGELOG.md` by default), so parallel branches that only append changelog bullets can all land without invoking the resolver. Genuine semantic conflicts still fall through to the cross-family resolver, and retained conflicts now carry a resolver witness explaining whether selection/spawn failed or the resolver ran but left markers.

- **Dispatch in-flight idempotency guard (Task 286).** A second dispatch of `{project, item_id}` (via `dispatch-task`, `dispatch-bundle` / Batch, direct `Worker.enqueue`, or cron poller) while a non-terminal run/job exists now returns the existing run's id and the prior job (`conflict?: true`) instead of enqueuing a duplicate. Terminal prior jobs do not block re-dispatch (recovery verbs unaffected). Centralized the unique spec (`unique_opts/0`) and job construction (`new_dispatch_job/4`, accepting Item or bare id) in `Run.Worker`; both enqueue sites and poller now share it. Added coverage in oban_dispatch_test (double-dispatch while executing returns live; terminal redo allowed) and batch_test (uniqueness spec applied); review commit hardened the long-running test timeout (30s→60s) and deduplicated the local helper to the public `Worker.unique_opts()`. Closes the double-dispatch root cause observed on ccxt_extract/146.

- **Dispatch idempotency for settled, unlanded retained-branch runs (Task 287).** A fresh dispatch for a task that already has a reviewer-approved (`:done, verdict: :approve, landed_sha: nil`) run record whose `harness/<run-id>` branch still exists in the project (the exact state left by an autonomous land conflict after the resolver agent failed) now refuses to enqueue or spawn duplicate work and returns `{:ok, existing_run_id, %Oban.Job{conflict?: true}}` (synthetic completed job to preserve caller shapes) instead. A warning log steers the caller to `dispatch-reland` (zero-token recovery of the retained, paid-for branch). Live non-terminal jobs (Task 286) and settled records that are landed or whose branch is gone do not block re-dispatch. Implemented in `Run.Worker` via the early `recoverable_settled_run/4` check (ResultStore list + `retained_run_branch?` git rev-parse on the project's local clone) before the normal new-dispatch + Oban unique path; lookup or branch absence falls through cleanly (logged at warning). Added targeted coverage in oban_dispatch_test. Coupled lander change: `Lander.Resilience.plan/2` now maps any `{:conflict, _}` (post-resolver-fail) to `{:conflict_retain, _}` unconditionally (no attempt-cap redispatch); `conflict_retain/2` marks the task blocked, emits Notification + OpsFeed witness, and returns `{:cancel, {:conflict_retained, reason}}`. `manual_reland_conflict` and `push_rejected` reland paths unchanged. Docs (agent-gate-workflow, CLAUDE, harness-driver skill via the assignee work) and resilience tests updated to match the "retain + recover, never re-dispatch" contract. Enforces the recover-don't-redo invariant for reviewer-approved work.

- **No-`adapter` dispatch now honors a task's pinned `assignee` instead of overriding it with `dispatch.default_agent`.** `dispatch-task` defaults `adapter` to the `recommend` sentinel, whose resolution went straight to `CapabilityScore.recommend` — and its no-data fallback is `dispatch.default_agent`. The task's roadmap-pinned `assignee` was never consulted (the `%Roadmap.Item{}` didn't even carry it), so three `ccxt_client` tasks pinned `codex`/`grok` were all routed to the default `cursor`: cursor ran the carried-over pinned model (`gpt-5.5` → usage-cap, `grok-build` → "cannot use this model"), produced a zero diff, and the opus reviewer silently rebuilt each from scratch — burning opus tokens and the capped cursor budget. Fix: `Item` now carries the parsed `assignee` (executable agents only; `human`/unknown → `nil`), and the recommend path **short-circuits to the pinned assignee when present** — capability scoring + the default-agent fallback only fill the gap for genuinely unpinned tasks. Coupled model fix: a pinned `model` applies only on its own assignee's adapter (`effective_model` guards `assignee == agent`); a cross-agent dispatch (explicit override, or no pin) uses the resolved agent's configured `agent_model` default, so an override can never carry a budget-capped/invalid agent+model pair. Regression tests cover pin-wins, no-pin-falls-through, and the existing explicit-bypass/scout-backed paths. Precedence decided in `Harness.Dispatch.recommended_adapter_for_item/3`; driver skill updated.
- **Post-merge audit invocation now threads the auditor agent's configured model — model-capable auditors no longer hard-fail with `{:model_required, _}`.** `Harness.Audit`'s `invocation/5` built the `%Invocation{}` with no `model`, so every audit ran with `model: nil`. Once the per-dispatch model guard landed (`AgentAdapter.invoke/2` rejects `nil` for any model-capable agent), every post-merge audit on a model-capable auditor (codex/claude/cursor/grok/pi) errored out with `{:model_required, Harness.AgentAdapter.Codex}` before the agent ran — best-effort, so nothing landed wrong, but the hygiene pass silently no-op'd (observed on `ccxt_client`). `invocation/6` now threads `auditor_model/1`, which resolves the chosen auditor agent's `{:agent_model, agent}` default (the same source the reviewer uses — audit has no task-pin axis). A registry-unmappable module (test-double `:auditor` overrides) yields `nil`, which is fine since those doubles are model-incapable; a real model-capable auditor with no configured default still surfaces `{:model_required, _}` rather than running on a sticky CLI default. Two regression tests cover the resolution and the unmappable-double nil.
- **Dashboard "unmerged only" run-history filter no longer shows settled-but-landed trash.** The filter rejects any run with `landed_sha == nil`, and `reconcile_history_entry/4` backfills that field by checking the honest git fact (is the task's `shipped_in` SHA — or the run's branch — an ancestor of `origin/<target>`?). Its guard previously matched only `state: :done, review_verdict: :approve`, so a **failed** run whose work nonetheless landed (manual salvage, `dispatch-rereview` on the retained branch, or a sibling retry that shipped the task) never reconciled and sat in "unmerged only" forever. The guard now matches any settled run (`state in [:done, :failed]`); `ResultStore.reconcile_landed_sha/4` remains the gate, so failed-but-landed runs drop out while genuinely-unmerged failures (task never shipped, branch gone) correctly stay. Two regression tests cover the failed-landed and failed-open cases.
- **Reviewer-stage staging no longer fatals on a gitignored `.harness/`; `.harness-retained` no longer leaks into commits (Task 282).** `Worktree.do_commit/2` + `diff_size/1` previously staged with `git add -A -- . :(exclude).harness`, which exits 1 whenever an *explicit* pathspec (even an `:(exclude)` one) matches a gitignored path — so in a repo that gitignores `.harness/` (harness's own shape) the reviewer's untracked `.harness/review.json` fataled the whole stage as `review_stuck`, bouncing the task back to pending before its verdict could be read (observed: run-1781425518611-74749b75 / task 281). Staging is now two mechanical moves (`stage_worktree/1`): a *bare* `git add -A` (no pathspec, so git silently skips the gitignored dir instead of erroring) then a `git reset` that unstages the full `@artifact_paths` family — covering both a *non*-gitignored `.harness/` and the never-gitignored `.harness-active`/`.harness-retained` markers, which the old single-`.harness` exclude let leak into agent commits. Read-only `status`/`diff` calls filter the same family via `@stage_exclude`. Verified across both repo states (gitignores `.harness/` and not).
- **Dashboard live_runs fan-out no longer stalls on a busy run gen_statem (Task 242).** `StatusView.live_runs/0` (and the LiveView feed) now does a bounded `Run.status(run_id, 100)` per run and skips on `:timeout` (or not_found); a single slow/stuck run gen_statem can no longer block the entire Active runs render. Test coverage for the skip path; the cap is a mechanical safety bound only.
...
- **Language-aware rule injection re-threaded from %Project{} (Task 288).** Restores the `language` signal (from Task 124) that regressed when CheckStack was deleted. `%Harness.Project{}` now carries `language` (optional atom; nil/:elixir keeps Elixir rule sections, others suppress them via RulesInjection). Wired through Dispatch.register_project/7 + its MCP schema (with presence test), ProjectRegistry build/fetch, struct, Project moduledoc; Run threads it to implementer/recovery/reviewer Invocations (verified by LanguageCaptureAdapter lifecycle tests); RulesInjection.render_opts + channel tests cover exclusion for :rust/:typescript etc. Review commit (9749540) tightened the Invocation @typedoc and added the MCP schema gate. CLAUDE.md updated in delivery; AGENTS.md drift fixed in post-merge audit.
