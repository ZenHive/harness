...
- **Agent KPI ledger split to `/harness/kpi/agents` (Task 298).** The wide sortable per-agent ledger, including dynamic reviewer-rating columns, now lives on its own dedicated page. `/harness/kpi` remains the overview with fleet headline stats and secondary KPI sections, links through to the full ledger, and the new ledger page links back. LiveView tests cover the new route, empty state, navigation, ledger rendering, and sorting.
