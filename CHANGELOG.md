...
- **Roadmap reads and writes sync the checkout before rmap runs (Task 424).** `Harness.Roadmap.ingest/2` (and `next_bundle` / `ready` / the `dispatch-*` MCP tools behind them) and the durable `mark_*` writeback now fetch and fast-forward the `roadmap_path` checkout onto its roadmap branch via `Harness.Git.TargetSync.sync_checkout/2` before shelling out to `rmap`. Dispatch reads the tasks that are actually on origin instead of whatever the node's checkout last saw, and the writeback push is a fast-forward. A dirty, diverged, detached, self-host, or non-git checkout is left alone with a witnessed skip and the call proceeds on the on-disk state — the sync is never a gate.

- **Cron polling no longer re-dispatches tasks that already landed (Task 419).** `Harness.Cron.RoadmapPoller` consults a synced view of the roadmap branch before building the ready set, so a task the lander already marked `done --shipped-in` on origin cannot be read as still pending from a stale local checkout. Observed on bourse 2026-08-28/29: tasks 687, 688 and 690 each landed four to five times over roughly eight hourly polls, spending twenty-odd implementer and reviewer invocations redoing work already on origin. `Git.TargetSync` still never writes the operator's working tree.
