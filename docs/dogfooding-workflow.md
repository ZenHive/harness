...
| `:failed` / `{:review_rejected, report}` | The reviewer found nothing salvageable (degenerate case — rejection is near-never by design). | Read `report` and retained-branch evidence; explicitly choose resume, rereview, fresh or defer. |
| `:failed` / `{:review_stuck, report}` | The gate could not produce a verdict: no cross-family reviewer available, reviewer crashed, or it exited without writing a readable `.harness/review.json`. | Read `report`; choose recovery or defer while the environment is repaired. |
