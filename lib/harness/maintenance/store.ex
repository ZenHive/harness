defmodule Harness.Maintenance.Store do
  ...
  alias Harness.Maintenance.Document
  alias Harness.Repo
  alias Harness.Store.EtsHeir

  @table __MODULE__

  @doc "Whether maintenances survive a BEAM restart."
  @spec persistent?() :: boolean()
  def persistent?, do: Application.get_env(:harness, :repo_enabled, true)

  @doc "Fetches one maintenance document."
  ...
  def serialized(fun) do
    if persistent?() do
      Repo.checkout(fn -> locked(fun) end, timeout: 3_660_000)
    else
  ...
  defp locked(fun) do
    %{rows: [[locked]]} = Repo.query!("SELECT pg_try_advisory_lock(444, 1)")

    if locked do
      try do
        fun.()
      after
        Repo.query!("SELECT pg_advisory_unlock(444, 1)")
      end
    else
      {:error, :already_sweeping}
    end
  end
  ...
  defp ensure_table do
    if :ets.whereis(@table) == :undefined do
      heir = EtsHeir.pid(Harness.Maintenance.Store.Heir)
