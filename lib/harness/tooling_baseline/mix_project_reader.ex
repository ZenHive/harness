defmodule Harness.ToolingBaseline.MixProjectReader do
  ...
  @spec read(String.t()) :: {:ok, %{deps: MapSet.t(String.t()), aliases: MapSet.t(String.t())}} | {:error, term()}
  ...
  @spec extract_deps(Macro.t()) :: MapSet.t(String.t())
  ...
  @spec extract_aliases(Macro.t()) :: MapSet.t(String.t())
  ...
  @spec dep_name(term()) :: String.t() | nil
  defp dep_name({:{}, _, [name | _]}) when is_atom(name), do: Atom.to_string(name)
  defp dep_name({name, _second}) when is_atom(name), do: Atom.to_string(name)
  defp dep_name({name, _, _} = tuple) when is_atom(name) and is_tuple(tuple), do: Atom.to_string(name)
  defp dep_name(name) when is_atom(name), do: Atom.to_string(name)
  defp dep_name(_other), do: nil

  @spec alias_name(term()) :: String.t() | nil
  defp alias_name({name, _tasks}) when is_atom(name), do: Atom.to_string(name)
  defp alias_name(name) when is_atom(name), do: Atom.to_string(name)
